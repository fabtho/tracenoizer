...
sub search{ 
    ...
    for (my $r = 0;$r < 10;$r++) {  
	$keyword_infog[$r] =  $keyword_infog[$r] - $keyword_infog[9];
...
sub makemodel{
 statussetz(3);
    print "/usr/local/bin/rainbow --append-stoplist-file=stoplist -H -d model -i $main_id/*";
    `/usr/local/bin/rainbow --append-stoplist-file=stoplist -H -d model -i $main_id/*`;
