...
sub search{   
    ...
    my $searchmachine = onelanguage("de");
   my $k = linkload($searchmachine,$searchterm,"de");
    
    $searchmachine = onelanguage("en");
    $k = $k + linkload($searchmachine,$searchterm,"en");
  ...
  my @searchterm= split(/ /,$searchterm);
    ...
    $searchmachine = onelanguage("de");
    $k = $k + linkload($searchmachine,$searchterm,"de");
    
    $searchmachine = onelanguage("en");
    $k = $k + linkload($searchmachine,$searchterm,"en");  
    print "---> $k urls found and put into aturl \n";
    if ($k == 0) {
	print "keinlick\n";
	statussetz(9);
	exit;
...
sub ftpup{    
	...
	foreach(@word){
	    my $filename = $_.".html";
	    if ($fehler == 0){
		$ftp->put("/home/tracenoizer/traceintern/art/$main_id/$filename") or $fehler = 6;      
	    }
	}
