...
# $ARGV[0] = $old_main_id, $ARGV[1] = $main_id,   $ARGV[2] usr_id


onesearch($ARGV[0],$ARGV[1],$ARGV[2]);
...
######################### sub onesearch() ###############################################################
#startet die such ueber einen namen der den status name hat, dh 
...
sub onesearch{
    ...
    $main_id = $_[1];
    my $usr_id = $_[2];
    my $abfrage= $dbh->prepare("select in_keyword from input where in_main_id = $old_main_id");
    $abfrage->execute;
    my $usr_name = $abfrage->fetchrow_array;   
   # my @usr_name = split(/ /,$usr_name);
   # $usr_name= $usr_name[1]." ".$usr_name[0];
     
    my $input = $dbh->prepare("insert into input (in_main_id,in_keyword) values ($main_id,'$usr_name')");
    $input->execute;
    print "----> insert into input (in_main_id,in_keyword) values ($main_id,klon) \n";

    #print "---->the ID = $main_id and person $name_vorname \n";
    statussetz(0);
    search($old_main_id,$usr_id);
}
...
sub search{ 
    ...
    my $abfrage= $dbh->prepare("select in_keyword from input where in_main_id = $old_main_id");
    $abfrage->execute;
    my $searchterm = $abfrage->fetchrow_array;
    
    
    my $abfrage= $dbh->prepare("select main_generation from main where main_id = $main_id");
    $abfrage->execute;
    my $main_generation = $abfrage->fetchrow_array;
 
    my $stillask = 8 - $main_generation;
    
    print "old_main_id $old_main_id\n";  

    $abfrage= $dbh->prepare("select keyword,keyword_infog from keyword where keyword_main_id = '$old_main_id' order by keyword_infog desc");
    $abfrage->execute;
    for (my $r = 0;$r < 10;$r++) {       
	my @antwort = $abfrage->fetchrow_array;
	$keyword[$r] = $antwort[0];
	print "  $antwort[0]  $antwort[1]\n";     
	$keyword_infog[$r] = $antwort[1];
	print " allinfog :: $allinfog\n";     
    }
    for (my $r = 0;$r < 10;$r++) {  
	$keyword_infog[$r] =  $keyword_infog[$r] - $keyword_infog[9] + 0.03;
	$allinfog = $allinfog + $keyword_infog[$r];
	print " allinfog :: $allinfog\n";
    }
    
    if ($stillask > 0) {
	
	my $locallinknumber = $stillask * 6;

        print "locallinknumber $locallinknumber \n";   

	$linknumber = $linknumber - ( 2 * $locallinknumber) ;
	print "linknumber $linknumber \n"; 
  
	$searchmachine = onelanguage("al");
	$t = $t + linkload($searchmachine,$searchterm,"al",$locallinknumber);
	my @searchterm= split(/ /,$searchterm);
	$searchterm = $searchterm[1]." ".$searchterm[0];
	print "---> newsearchterm : $searchterm  \n";
	$searchmachine = onelanguage("al");
	$t = $t + linkload($searchmachine,$searchterm,"al",$locallinknumber);
    }
    my $infogfaktor = $linknumber/$allinfog;

    for (my $r = 0;$r < @keyword_infog;$r++) {
        $keyword_infog[$r] =  $infogfaktor * $keyword_infog[$r];
        print "keyword_infog $keyword_infog[$r]\n";
    }  
    
    for (my $r= 0;$r < 10;$r=$r+2) {        
	my $searchterm = "+".$keyword[$r]."+".$keyword[$r+1];
	my $locallinknumber = int($keyword_infog[$r] + $keyword_infog[$r+1])+1;
	my $searchmachine = onelanguage("al");
	print "locallinknumber $locallinknumber \n";
	$t = $t + linkload($searchmachine,$searchterm,"al",$locallinknumber);          
	print "---> $t urls found and put into aturl \n";      
    }                              
    print "---> $t urls found and put into aturl \n";
    if ($t == 0) {
	print "keinlink\n";
...
sub all_page{
	system ("rm -R /home/tracenoizer/traceintern/$main_id/*");
...
sub start_page{
#weil immer nicht fertig statusetze schon 7 hier
	statussetz(7);
    my @allthemas = @word;
    my $suchwort = $_[0];
    my $image = "";
...
sub on_page{
    ...
    $seite = $seite."<meta name=\"Date_modified\" content=\"2001-06-14\">\n";
    $seite = $seite."<meta name=\"ROBOTS\" content=\"index,follow\">\n";
    $seite = $seite."</head>\n"; 
    $seite = $seite."<style type=\"text/css\"> 
	A.nav:link {TEXT-DECORATION: none}
...
sub fileup{
        ...
        system ("cp -r /home/tracenoizer/traceintern/$main_id /home/werner/public_html/");
          ...
          #  $update->execute;
           ...
           # if ($ac_used > 30){
               # my $update = $dbh->prepare("update accounts set ac_used = 1 where ac_id = $ac_id");
                #$update->execute;
                #mailsend($ac_http," dieser account ist voll");
...
sub makestartindex{
    ...
    $ac_indexfile = $ac_indexfile ."<meta name=\"Date_modified\" content=\"2001-07-13\">\n";
