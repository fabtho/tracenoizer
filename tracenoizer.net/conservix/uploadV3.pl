...
sub fileup{
        system ("cp -r /var/www/temp/$main_id /var/www/pages/");
        #system ("cp -r /home/tracenoizer/public_html/temp/$main_id /home/werner/public_html/");
	$ac_http = "http://www.freepages.ath.cx-cdrom/pages/$main_id/index.html";
...
sub makestartindex{
	open    (INDEX,"</var/www/pages/index.html");
        my $index = ();
        $i = 1;
        while(<INDEX>){
        	$index = $index.$_;
        if ($i==2){
        	$index = $index.'<a href="http://www.freepages.ath.cx-cdrom/pages/'.$main_id.'/index.html">Freepage '.$main_id."</a><br>\n";
        #print $index;
	}
	#print $i."\n";
        $i++;
	...
	#open (INDEX,">/home/werner/public_html/index.html");
       print $index;
        print INDEX $index;
        close (INDEX);
 }
