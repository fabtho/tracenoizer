...
//get status descriptions
	$stati= mysql_db_query("tracenoizer_new", "select st_string from status");
...
$query2 = ("SELECT main_date, main_weblink, main_ac_id, main_st_id, main_id FROM main WHERE main_usr_id = $userID ");
	...
	if($state >= 6){//value needs to be changed if number of states changes (to the case where clone is active)
					$url= 'http://tracenoizer.org-cdrom/temp/'.$nr.'/index.html';
					echo '
					<a name="'.$nr.'"></a>
					...
					if ($state == 6) {// lokal
						...
						echo '
						Your Site : <a href="http://tracenoizer.org-cdrom/temp/'.$nr.'/index.html" class="nav" target="_new">http://tracenoizer.org-cdrom/temp/'.$nr.'/index.html</a><br>
						<font size="2">link opens in a new window</font><br><br>
						<b>Composition:</b><br>
						<script language="JavaScript">
						//alert("es tut");
							F1 = window.open("http://tracenoizer.org-cdrom/popupV3.php?url='.$url.'&usr_id='.$userID.'&main_id='.$nr.'","clonesite","width=800,heigth=600,resizable=yes");

//alert("http://tracenoizer.org-cdrom/popupV3.php?url='.$url.'&usr_id='.$userID.'&main_id='.$nr.'");
 						</script>
 						';
