...
sub makemodel{
 ...
 print "----> start to make the statistic model\n";
    ...
    print "/usr/local/bin/rainbow --append-stoplist-file=/var/www/traceintern/stoplist -H -d model -i temp/$main_id/*";

#mkdir("/var/www/public_html/model",0777) || print "----> konnte dir /var/www/public_html/model nicht machen: $!\n";
# workaround
# system "wget http://tracenoizer.org-cdrom/modelanleg.php?main_id=$main_id";
#system "rainbowstart" || "fehler $!";

#`perl modelmake.pl $main_id`;

system "/usr/local/bin/rainbow --append-stoplist-file=/var/www/stoplist -H -d /var/www/model -i /var/www/temp/$main_id/*" || "fehler$!\n";
