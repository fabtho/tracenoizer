...
;; Author: Christoph Conrad <christoph.conrad(at)gmx.de>
...
;; - David Biesack <sasdjb(at)unx.sas.com>
;; - Peter Breton <pbreton(at)bluewin.ch> (send me his own `which' code)
;; - Mickey Ferguson <MFerguson(at)peinc.com> (`file-executable-p' on Windows
;;   NT is curious)
;; - Kai Groﬂjohann <Kai.Grossjohann(at)CS.Uni-Dortmund.DE>. He published
;;   under the subject "Re: where-is" a snippet of code with the basis
;;   functionality.
;; - Bruce Ingalls <bingalls(at)users.sourceforge.net>
;; - Jonas Steverud <d4jonas(at)dtek.chalmers.se>
...
(defconst which-cvs-revision "$Revision: 1.21 $")
...
;; $Log: which.el,v $
;; Revision 1.21  2003/05/24 17:37:01  bingalls
;; v2.7.2 alpha
...
;; beta will be tested with tramp
;;
;; Revision 1.2  2000/07/10 20:20:08  cc
;; UPD: Version: 0.44
;; FIX: Mailadress corrected to bingalls(at)users.sourceforge.net
...
;; 15 Jun 2000 - v0.42:
;; - changed email address to "christoph.conrad(at)gmx.de"
